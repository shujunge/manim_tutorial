...
class ManyFourierCirclesScene(Scene):
    """
    使用傅里叶变换同时绘制多个图标
    """
    CONFIG = {
        ...
        "base_frequency": 1,
        "slow_factor": 0.25,
        "center_point": [2*LEFT, ORIGIN, 2*RIGHT],
        "parametric_function_step_size": 0.001,
        "drawn_path_color": YELLOW,
        "drawn_path_stroke_width": 2,
    }
    ...
    def get_rotating_vectors(self, center_point, freqs=None, coefficients=None):
        vectors = VGroup()
        self.center_tracker = VectorizedPoint(center_point)
    ...
    # Computing Fourier series
    # i.e. where all the math happens
    def get_coefficients_of_path(self,center_point,  path, n_samples=10000, freqs=None):
        if freqs is None:
            freqs = self.get_freqs()
        dt = 1 / n_samples
        ts = np.arange(0, 1, dt)
...
class ManyFourierOfTexSymbol(ManyFourierCirclesScene):
    CONFIG = {
        "n_vectors": 51,
        "center_point": [3*LEFT, 1.5*LEFT, ORIGIN, 1.5*RIGHT],
        "slow_factor": 0.1,
        "n_cycles": 2,
        "tex": ["\\pi", "A", "\\sum", "\\sigma"],
        "scale":[3, 2, 1, 2],
    ...
    def add_vectors_circles_path(self):
        ...
        for index, center_point in enumerate(self.center_point):
            tex_mob = TexMobject(self.tex[index]).scale(self.scale[index]).shift(center_point)
...
class ManyFourierOfTextSymbol(ManyFourierCirclesScene):
    CONFIG = {
        "n_vectors": 51,
        "center_point": [4.5*LEFT, 3*LEFT, 1.5*LEFT, ORIGIN, 1.5*RIGHT, 3*RIGHT, 4.5*RIGHT, 6*RIGHT],
        "slow_factor": 0.1,
        "n_cycles": 1,
        "tex": [index for index in "西安电子科技大学"],
        "start_drawn": False,
        "max_circle_stroke_width": 1,
    }
    ...
    def add_vectors_circles_path(self):
        ...
        for index, center_point in enumerate(self.center_point):
            tex_mob = Text(self.tex[index]).scale(scale[index]).shift(center_point)
            path = tex_mob.family_members_with_points()[0]
            path.set_fill(opacity=0)
            path.set_stroke(WHITE, 1)
            object_list.add(path)
