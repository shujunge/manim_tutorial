...
class TridentOfNewton(GraphScene):
    ...
    def construct(self):
        ...
        parabola_graph.add_updater(
            lambda x: x.become(self.get_graph(lambda x: aa.get_value() * x ** 2, x_max=20, color=RED_A)))
        ...
        inverse_function_graph = VGroup(
            inverse.add_updater(
                lambda x: x.become(self.get_graph(lambda x: bb.get_value() / x, x_min=0.05, color=PURPLE_E))),
            self.get_graph(lambda x: bb.get_value() / x, x_max=-0.05, color=PURPLE_E).add_updater(
                lambda x: x.become(self.get_graph(lambda x: bb.get_value() / x, x_max=-0.05, color=PURPLE_E)))
        )
        inverse_function_graph_label = self.get_graph_label(inverse, label="g(x)={b\\over x}", direction=UP,
                                                            buff=MED_LARGE_BUFF)
        self.play(
        ...
        trident_of_newton_l.add_updater(lambda x: x.become(
            self.get_graph(lambda x: aa.get_value() * x ** 2 + bb.get_value() / x, x_min=0.05, color=YELLOW)))
        trident_of_newton_r = self.get_graph(lambda x: a * x ** 2 + b / x, x_max=-0.02, color=YELLOW)
        trident_of_newton_r.add_updater(lambda x: x.become(
            self.get_graph(lambda x: aa.get_value() * x ** 2 + bb.get_value() / x, x_max=-0.05, color=YELLOW)))
...
class Analysis(Scene):
    def construct(self):
        ...
        text2 = TextMobject(
            r"当$x\rightarrow 0$时,$y \rightarrow \infty$,图像趋近于反比例函数图像,\\当$x \rightarrow \infty$时,图像趋近于抛物线").next_to(
            text1, DOWN)
        self.play(
            Write(text1),
            Write(text2),
...
class Dir(TridentOfNewton):
    def construct(self):
        ...
        dot_label = TexMobject(r"({\sqrt[3]{{b}\over{2a}}},{3\sqrt[3]{{ab^2}\over{4}}})").move_to(
            np.array([2, 1, 0])).scale(0.6)
        self.play(
            Write(line),
            Write(dot),
            Write(dot_label),
...
class End(Scene):
    def construct(self):
        text = TextMobject(
            r"感谢观看！\\BGM:Last Train Home\\Tools:Davinci Resolve, Visual Studio Code, manim\\by emat").to_edge(UP)
        self.play(
            Write(text)
        )
        self.wait(4)
